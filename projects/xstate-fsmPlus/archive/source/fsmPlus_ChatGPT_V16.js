/*
  xstate-fsmPlus.js - Hierarchical FSM for Espruino V16
...
function findTransition(state, eventObject, stateLookup) {
  let currentState = stateLookup[state.value];

  while (currentState) {
    if (currentState.on && currentState.on[eventObject.type]) {
      return currentState.on[eventObject.type];
    }
    const parentStateId = state.value.includes('.') ? state.value.split('.').slice(0, -1).join('.') : null;
    currentState = parentStateId ? stateLookup[parentStateId] : null;
  }
...
function createMachine(fsmConfig, options) {
  ...
  var machine = {
    config: fsmConfig,
    _options: options,
    _stateLookup: stateLookup, // Store stateLookup in the machine instance
    initialState: {
      value: initialResolved,
      actions: [],
      context: fsmConfig.context,
      matches: (target) => target === initialResolved
    },
    transition: function (state, event) {
      var eventObject = toEventObject(event);
      var currentState = this._stateLookup[state.value];
      ...
      var transition = findTransition(state, eventObject, this._stateLookup);
      ...
      var targetResolved = this._stateLookup[transition.target] ? this._stateLookup[transition.target].initialResolved : transition.target;
      ...
      return {
        ...
        matches: (target) => target === targetResolved
      };
    }
  };
  return machine;
